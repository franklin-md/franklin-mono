...
import type { NetworkConfig, FilesystemConfig, EnvironmentConfig } from '@franklin/extensions';
...
export class SandboxedTerminal implements Terminal {
    ...
    setCwd(cwd: string) {
        this._cwd = cwd;
        this._config.filesystem.denyWrite = ['.env', `${cwd}/auth.json`, `${cwd}/sessions`, `${cwd}/store`]
    }
    ...
    constructor(config: EnvironmentConfig) {
        ...
        this._config = {
            ...
            filesystem: {
                ...
                denyWrite: ['.env', `${cwd}/auth.json`, `${cwd}/sessions`, `${cwd}/store`]
            }
        }
        this._cwd = cwd;
    }
    ...
    async exec({cmd, timeout}: TerminalInput) {
            ...
            const child = spawn(sandboxedCommand, { shell: true, stdio: 'pipe', cwd: this._cwd, timeout })
