 ...
 *
 * `A` is the API family. `createApi` binds that family to the chosen
 * `ContextRuntime`; callers should use the compile helpers/combinators so
 * API context and build runtime stay coordinated.
 */
export type Compiler<A extends API, Runtime extends BaseRuntime & A['In']> = {
	...
	readonly createApi: <ContextRuntime extends Runtime>() => BoundAPI<
		A,
		ContextRuntime
	>;
