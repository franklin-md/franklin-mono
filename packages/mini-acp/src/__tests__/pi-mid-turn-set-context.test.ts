...
function piAssistantTexts(messages: PiMessage[]): string[] {
	return messages.flatMap((message) => {
...
describe('Pi setContext during an active Mini-ACP turn', () => {
	it('makes tools registered by one tool call available to the next Pi LLM call', async () => {
		...
		const result = await collect(
			client.prompt({
				role: 'user',
				content: [{ type: 'text', text: 'Open the vault and get the money.' }],
			}),
		);
	...
	it('keeps assistant updates in context for the next Mini-ACP prompt', async () => {
		...
		const streamFn: StreamFn = (model, context) => {
			observations.push({
				assistantTexts: piAssistantTexts(context.messages),
				userTexts: piUserTexts(context.messages),
			});
			...
			return streamDone(
				assistantMessage(
					model,
					[{ type: 'text', text: `reply-${observations.length}` }],
					'stop',
				),
				'stop',
			);
		...
		expect(observations[0]?.userTexts).toEqual(['first prompt']);
		expect(observations[0]?.assistantTexts).toEqual([]);
		expect(observations[1]?.userTexts).toContain('first prompt');
		expect(observations[1]?.userTexts).toContain('second prompt');
		expect(observations[1]?.assistantTexts).toContain('reply-1');
	});
});
