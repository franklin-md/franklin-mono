...
import { endBlock, endTrailingSequentialBlock } from './end.js';
 ...
 * block — callers that want the "close trailing sequential content before
 * opening" path should use startNewBlock.
 *
...
/**
 * Close the trailing sequential block (if any is still open) and push a new
 * block. Open toolUse blocks are lifecycle-bound to matching tool results, so
 * they intentionally survive later blocks.
 *
...
/**
 * Push an instantaneous block: close trailing sequential content, start a new
 * one, and end it at the same moment. Used for point-in-time events like a
