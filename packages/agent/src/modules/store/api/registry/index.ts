 ...
 *
 * Runtimes hold references (refs) into the registry rather than owning
 * stores directly. This decouples store lifecycle from runtime lifecycle
...
export class StoreRegistry {
	...
	create(sharing: Sharing, initial?: JsonValue): StoreEntry {
		...
		const store = new BaseStore<unknown>(initial);
	...
	async restore(): Promise<RestoreResult> {
		...
		for (const [ref, { value, sharing }] of values) {
			this.add({ ref, sharing, store: new BaseStore<unknown>(value) });
		}
		return { issues };
	}
