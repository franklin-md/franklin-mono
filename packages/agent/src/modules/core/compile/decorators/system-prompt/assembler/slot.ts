 ...
 * membership + pinning). Synchronous; runs inside the handler's
 * `setPart` closure. Content resolution is deferred to
 * {@link resolveSlotContent} so that async factories can be awaited after
 * the handler returns.
 *
 * `cache` is re-declared on every call (it resets to its default when
 * omitted), consistent with the per-call options-bag model. `pinned` is
 * sticky once set.
 */
